...
void mixed_operations(Table& table, int numOps) {
    std::mt19937 rng(std::random_device{}());
    std::uniform_int_distribution<int> keyDist(0, MAX_KEY);
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        try {
            initial_capacity = std::stoul(argv[1]);
        } catch (...) {
            std::cerr << "Invalid capacity argument, using default (128)." << std::endl;
        }
    }
